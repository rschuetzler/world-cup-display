...
bool parseScoreboard(char* buf, size_t len, std::vector<Match>& out) {
  ...
  // The ESPN payload nests a large `leagues` blob (calendar, logos, links…)
  // ahead of `events`; ArduinoJson still descends it to skip it under the
  // filter, so the default nesting limit of 10 isn't enough — raise it.
